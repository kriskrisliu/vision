...
# use hawq codes to quant
def hawq_quant(model):
    print("-"*80)
    # quant transformer
    weights_list = ["attn.qkv","attn.proj", "mlp.fc1","mlp.fc2"]
    # quant head
    head = getattr(model,'head')
    qlinear = QuantLinear()
    setattr(qlinear,'output_layer',True)
    qlinear.set_param(head)
    update_module(model, 'head', new_module=qlinear)

    # quant patch_embed_proj
    patch_embed_proj = getattr(model,"patch_embed").proj
    # print(patch_embed_proj.__dict__)
    qconv = QuantConv2d()
    qconv.set_param(patch_embed_proj)
    update_module(model, 'patch_embed.proj', new_module=qconv)

    # quant others
    for idx, module in enumerate(model.blocks):
        for name_end in weights_list:
            head_name = name_end.split('.')[0]
            tail_name = name_end.split('.')[1]
            mm = getattr(module, head_name)
            mm = getattr(mm, tail_name)
            qlinear = QuantLinear()
            qlinear.set_param(mm)
            quant_out_proj = qlinear
            module_name = f"blocks.{idx}.{name_end}"
            update_module(model, module_name, new_module=quant_out_proj)
    return model
...
class QuantAct(Module):
    ...
    def forward(self, x, pre_act_scaling_factor=None, pre_weight_scaling_factor=None, identity=None,
        ...
        if not self.full_precision_flag:
            ...
            # if self.ownname=='transformer.resblocks.0.mlp.c_proj.Qact':
            # if self.ownname=='transformer.resblocks.0.mlp.c_fc.Qact':
                # print("-"*80)
                # print(self.ownname)
                # print(x.shape)
                # print("x.min(),x.mean(),x.max():",x.min(),x.mean(),x.max())
                # print(torch.histogram(x.data.flatten().detach().cpu(), bins=100))
                # tmp = x.flatten().sort()[0]
                # for ii in range(tmp.shape[0]):
                #     print("{:.2f}".format(tmp[ii]))
                # torch.save(x, f"act_logs/{time.time()}.pt")
                # print("-"*80)
                # raise NotImplementedError
            # print(self.ownname,self.act_scaling_factor)
            # if self.ownname=="features.1.block.1.fc2.Qact":
                # print(quant_act_int)
                # return quant_act_int * correct_output_scale+11,0
...
class QuantConv2d(Module):
    ...
    def __repr__(self):
        ...
        s = "(" + s + " weight_bit={}, full_precision_flag={}, quant_mode={}, use_static={}, static_num={})".format(self.weight_bit,
    ...
    def forward(self, x, pre_act_scaling_factor=None):
        ...
        x = torch.clamp(x, -self.clip_point, self.clip_point)
        # print(self.clip_point)
        if self.use_static:
            x = x+self.static_num
        if self.use_noise and self.bias is None:
            noise = self.noise*(self.noiseScale)
            print(x.shape)
            print(noise.shape)
            x = x+noise

        x,act_scaling_factor = self.Qact(x,clip_point=self.clip_point)
        if self.use_static:
            x = x-self.static_num
        if self.use_noise:
            x = x-noise
        ...
        if self.bias is None:
            ...
            return (F.conv2d(x, self.weight_integer, None,
                             self.conv.stride, self.conv.padding, self.conv.dilation, self.conv.groups)
                    * correct_output_scale)
        else:
