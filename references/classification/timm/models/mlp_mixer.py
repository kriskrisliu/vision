...
from .helpers import build_model_with_cfg, overlay_external_default_cfg, named_apply, load_checkpoint
...
class MixerBlock(nn.Module):
    ...
    def forward(self, x):
        self.actr=[x.data.min(),x.data.max()]
        x = x + self.drop_path(self.mlp_tokens(self.norm1(x).transpose(1, 2)).transpose(1, 2))
        x = x + self.drop_path(self.mlp_channels(self.norm2(x)))
        return x

class multi_MixerBlock(nn.Module):
    """ Residual Block w/ token mixing and channel MLPs
    Based on: 'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    """
    def __init__(
            self, dim, seq_len, mlp_ratio=(0.5, 4.0), mlp_layer=Mlp,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), act_layer=nn.GELU, drop=0., drop_path=0.,tknum=4,cnnum=1,finetune=False):
        super().__init__()
        tokens_dim, channels_dim = [int(x * dim) for x in to_2tuple(mlp_ratio)]
    ...
    def forward(self, x):
        ...
        for i in range (self.token_num):
            y[i] = self.mlp_tokens[i](y[i])
            # print("aft mlp-token: ",x.data.min(),x.data.max())
        y=tuple(y)
        x=torch.cat(y,dim=1)
        x=x.transpose(1, 2) + identity
        ...
        for i in range(self.channel_num):
            y[i] = self.mlp_channels[i](y[i])
        # print("aft mlp-channel: ",x.data.min(),x.data.max())
        y=tuple(y)
        x=torch.cat(y,dim=1)
        x = x + identity
        return x
...
class multi_ResBlock(nn.Module):
    """ Residual MLP block w/ LayerScale and Affine 'norm'

    Based on: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    def __init__(
            self, dim, seq_len, mlp_ratio=4, mlp_layer=Mlp, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.linear_tokens = torch.nn.ModuleList([nn.Linear(seq_len, seq_len) for i in range(tknum)])
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        self.mlp_channels = mlp_layer(dim, channel_dim, act_layer=act_layer, drop=drop)
        self.ls1 = nn.Parameter(init_values * torch.ones(dim))
    ...
    def forward(self, x):
        ...
        y=list(y)
        for i in range (self.token_num):
            y[i] = self.linear_tokens[i](y[i])
            # print("aft mlp-token: ",x.data.min(),x.data.max())
        y=tuple(y)
        x=torch.cat(y,dim=1)
        x = self.drop_path(self.ls1*x.transpose(1, 2)) + identity
...
class MlpMixer(nn.Module):
    ...
    def forward_features(self, x):
        x = self.stem(x)
        # print('ref tensor:',x.data.max(),x.data.min())
        x = self.blocks(x)
        # print('ref tensor:',x.data.max(),x.data.min())
        min_act,max_act=0,0
        # for i in range(self.num_blocks):        
        #     min_act += self.blocks[i].actr[0]
        #     max_act += self.blocks[i].actr[1]
        # print("actrange: ",min_act/self.num_blocks,max_act/self.num_blocks)
        x = self.norm(x)
        x = x.mean(dim=1)
        return x
...
@register_model
def mixer_b16_224_relu(pretrained=False, **kwargs):
    """ Mixer-B/16 224x224. ImageNet-1k pretrained weights.
    Paper:  'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    """
    model_args = dict(patch_size=16, num_blocks=12, embed_dim=768, act_layer=nn.ReLU, **kwargs)
    model = _create_mixer('mixer_b16_224', pretrained=pretrained, **model_args)
    return model

@register_model
def mixer_b16_224_multi(pretrained=False, **kwargs):
    """ Mixer-B/16 224x224. ImageNet-1k pretrained weights.
    Paper:  'MLP-Mixer: An all-MLP Architecture for Vision' - https://arxiv.org/abs/2105.01601
    """
    model_args = dict(patch_size=16, num_blocks=12, embed_dim=768,block_layer=multi_MixerBlock, **kwargs)
    model = _create_mixer('mixer_b16_224', pretrained=False, **model_args)
    load_checkpoint(model,"./453.pth.tar",use_ema=True)
    return model
...
@register_model
def resmlp_24_224_relu(pretrained=False, **kwargs):
    """ ResMLP-24
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=384, mlp_ratio=4,
        block_layer=partial(ResBlock, init_values=1e-5), norm_layer=Affine, act_layer=nn.ReLU, **kwargs)
    model = _create_mixer('resmlp_24_224', pretrained=pretrained, **model_args)
    return model

@register_model
def resmlp_24_224_bn_relu(pretrained=False, **kwargs):
    """ ResMLP-24
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=384, mlp_ratio=4,
        block_layer=partial(ResBlock, init_values=1e-5), act_layer=nn.ReLU, **kwargs)
...
@register_model
def resmlp_24_224_bn(pretrained=False, **kwargs):
    """ ResMLP-24
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=384, mlp_ratio=4,
        block_layer=partial(ResBlock, init_values=1e-5), **kwargs)
    model = _create_mixer('resmlp_24_224', pretrained=False, **model_args)
    if pretrained:
...
@register_model
def resmlp_24_224_bn_pact(pretrained=False, **kwargs):
    """ ResMLP-24
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=384, mlp_ratio=4,
        block_layer=partial(ResBlock, init_values=1e-5), activation=partial(PactReLU, upper_bound=200.0), pretrained_strict=False, **kwargs)
    model = _create_mixer('resmlp_24_224', pretrained=False, **model_args)
    if pretrained:
        load_checkpoint(model,"./gelu_bn.pth.tar",use_ema=True)
    return model
...
@register_model
def resmlp_24_224_multibn(pretrained=False, **kwargs):
    """ ResMLP-24
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=384, mlp_ratio=4,
        block_layer=partial(multi_ResBlock, init_values=1e-5), **kwargs)
    model = _create_mixer('resmlp_24_224', pretrained=False, **model_args)
    if pretrained:
        load_checkpoint(model,"./checkpoint-533.pth.tar",use_ema=True)
    return model
...
def resmlp_big_24_224(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=768, mlp_ratio=4,
        block_layer=partial(multi_ResBlock, init_values=1e-6), **kwargs)
    model = _create_mixer('resmlp_24_224', pretrained=pretrained, **model_args)
    return model
