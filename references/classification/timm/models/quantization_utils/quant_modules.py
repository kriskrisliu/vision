...
class QuantLinear_lyj(Module):
    ...
    def __init__(self,
        ...
        super(QuantLinear_lyj, self).__init__()
        ...
        self.quant_mode = quant_mode
        self.counter = 0
        self.Qact = QuantAct_lyj(fixed_point_quantization=True).to(device='cuda')
        self.ownname=None
    ...
    def __repr__(self):
        s = super(QuantLinear_lyj, self).__repr__()
...
class QuantAct_lyj(Module):
    ...
    def __init__(self,
        ...
        super(QuantAct_lyj, self).__init__()
    ...
    def __repr__(self):
        return "{0}(activation_bit={1}, " \
               ...
               "Act_max: {5:.2f}) running_stat:{6}".format(self.__class__.__name__, self.activation_bit,
                                          self.full_precision_flag, self.quant_mode, self.x_min.item(),
                                          self.x_max.item(), self.running_stat)
    ...
    def forward(self, x, pre_act_scaling_factor=None, pre_weight_scaling_factor=None, identity=None,
                identity_scaling_factor=None, identity_weight_scaling_factor=None,clip_point=None):
        if type(x) is tuple:
            if len(x) == 3:
                channel_num = x[2]
        ...
        if self.running_stat:
            ...
            # not using running_stat
            if clip_point is None:
                self.x_min = x.min()
                self.x_max = x.max()
            else:
                self.x_min = -torch.tensor([clip_point]).cuda()
                self.x_max = torch.tensor([clip_point]).cuda()
...
class QuantLinear(Module):
    ...
    def ptminmax(self):
        self.amin,self.amax=self.weight.data.min(),self.weight.data.max()
        print("Linear weight:",self.amin,self.amax)

    def forward(self, x, prev_act_scaling_factor=None, noise=None):
        """
        using quantized weights to forward activation x
        """
        # print(self.__repr__())
        ...
        if not self.full_precision_flag:
            if self.quant_mode == 'symmetric':
                ...
                raise Exception('For weight, we only support symmetric quantization.')
            # print("Bias: ",type(bias_scaling_factor),bias_scaling_factor.shape)
            # print("fc: ",type(self.fc_scaling_factor),self.fc_scaling_factor.shape)
            # print(bias_scaling_factor)
            # print(self.fc_scaling_factor)
        else:
            w = self.weight
            b = self.bias

            self.fc_scaling_factor=torch.ones(1).cuda()
            bias_scaling_factor = torch.ones(1,1).cuda()
        ...
        if not self.full_precision_flag:
            ...
            out_noise_part = 0
            if noise!=None:
                out_noise_part = F.linear(noise.view(1,1,-1), weight=self.weight_integer*self.fc_scaling_factor.view(-1,1))
                return (ste_round.apply(
                    F.linear(x_int, weight=self.weight_integer, bias=self.bias_integer)) * correct_output_scale - out_noise_part, self.fc_scaling_factor)
            else:
                return (ste_round.apply(
                    F.linear(x_int, weight=self.weight_integer, bias=self.bias_integer)) * correct_output_scale, self.fc_scaling_factor)
        else:
...
class QuantAct(Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('x_var', torch.zeros(1))
        self.register_buffer('x_99pct', torch.zeros(1))
        self.register_buffer('x_999pct', torch.zeros(1))
    ...
    def ptminmax(self):
        # self.amin,self.amax=self.x_min(),self.x_max()
        print("Act min/max: ",self.x_min,self.x_max)
        print("Mean/Variance: ",self.x_mean,self.x_var)
        print("pct 99/999: ",self.x_99pct,self.x_999pct)
    ...
    def forward(self, x, pre_act_scaling_factor=None, pre_weight_scaling_factor=None, identity=None,
        ...
        if self.running_stat:
            if self.act_percentile == 0:
                ...
                # print(x.shape)
                # print(x.data.min().shape)
                # print(x.detach().contiguous().view(-1).shape)
                # print(x.detach().view(-1).shape)
            ...
            # the lower_percentile will be set to 0 in order to make sure the final x_min is 0.
                ...
                # x_min, x_max = get_percentile_min_max(x.detach().contiguous().view(-1), 0, self.act_percentile, output_tensor=True)
                x_min, x_max = get_percentile_min_max(x.detach().contiguous().view(-1), 100 - self.act_percentile, self.act_percentile, output_tensor=True)
            ...
            # print("act_range_momentum: "+str(self.act_range_momentum))
            if self.act_range_momentum==0:
                self.x_mean=torch.mean(x)
                self.x_var=torch.var(x)
                _, self.x_99pct =  x_min, x_max = get_percentile_min_max(x.detach().contiguous().view(-1), 100 - 99,
                                        99, output_tensor=True)
                _, self.x_999pct =  x_min, x_max = get_percentile_min_max(x.detach().contiguous().view(-1), 100 - 99.9,
            ...
            if (pre_act_scaling_factor is None) or (self.fixed_point_quantization == True):
                ...
                if identity is None:
                    ...
                    quant_act_int = fixedpoint_fn.apply(x, self.activation_bit, self.quant_mode,
                                                        ...
                                                        identity_weight_scaling_factor)
            correct_output_scale = self.act_scaling_factor.view(-1)
            # print("act: ",type(self.act_scaling_factor),self.act_scaling_factor.shape)
            return (quant_act_int * correct_output_scale, self.act_scaling_factor)
        else:
            return (x, torch.ones(1).cuda())
...
class QuantConv2d(Module):
    ...
    def forward(self, x, pre_act_scaling_factor=None):
        ...
        # print("conv:",type(self.conv_scaling_factor),self.conv_scaling_factor.shape)
...
class QuantAffine(nn.Module):
    def __init__(self,
                 ...
                 fix_flag=False,
                 weight_percentile=0):
        super(QuantAffine, self).__init__()
    ...
    def __repr__(self):
        s = super(QuantAffine, self).__repr__()
        s = "(" + s + " weight_bit={}, full_precision_flag={}, quant_mode={})".format(self.weight_bit,
                                                                                      self.full_precision_flag,
                                                                                      self.quant_mode)
        return s
    ...
    def set_param(self, aff):
        self.alpha = Parameter(aff.alpha.data.clone())
        try:
            self.beta = Parameter(aff.beta.data.clone())
        except AttributeError:
            self.beta = None
    ...
    def forward(self, x,pre_act_scaling_factor=None):
        ...
        if type(x) is tuple:
            pre_act_scaling_factor = x[1]
            x = x[0]
        # print(self.alpha.shape,x.shape)
        # print("++++++++++++++++++++++")
        if self.quant_mode == "symmetric":
            self.weight_function = SymmetricQuantFunction.apply
        elif self.quant_mode == "asymmetric":
        ...
        # perform quantization
        if self.quant_mode == 'symmetric':
            self.alpha_scaling_factor = symmetric_linear_quantization_params(self.weight_bit, w_min, w_max,
                                                                            False)
            self.alpha_integer = self.weight_function(self.alpha, self.weight_bit, self.alpha_scaling_factor)
            beta_scaling_factor = self.alpha_scaling_factor.view(1, -1) * pre_act_scaling_factor.view(1, -1)
            if self.quantize_bias and (self.beta is not None):
                self.beta_integer = self.weight_function(self.beta, self.bias_bit, beta_scaling_factor)
                # print(self.beta_integer.shape)
            else:
                self.beta_integer = None
            # print(type(self.beta_integer),self.quantize_bias,(self.beta))
        else:
            raise Exception('For weight, we only support symmetric quantization.')

        pre_act_scaling_factor = pre_act_scaling_factor.view(1, -1, 1)
        x_int = x / pre_act_scaling_factor
        correct_output_scale = beta_scaling_factor.view(1, -1, 1)
