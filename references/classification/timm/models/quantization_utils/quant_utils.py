...
class AsymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, specified_scale=None, specified_zero_point=None):
        ...
        new_quant_x = linear_quantize(x, scale, zero_point, inplace=False)
        n = 2 ** k - 1
        # print("bef clamp: ",new_quant_x.data.max(),new_quant_x.data.min())
        new_quant_x = torch.clamp(new_quant_x, 0, n)
        # print("new_quant_x: ",new_quant_x.data.max(),new_quant_x.data.min())
        ...
        # new lines
        new_x=linear_dequantize(new_quant_x,scale,zero_point)
        # print("x: ",x.data.max(),x.data.min())
        # print("new_x: ",new_x.data.max(),new_x.data.min())
    ...
    def forward(ctx, z, bitwidth, quant_mode, z_scaling_factor, case, pre_act_scaling_factor=None,
        ...
        with torch.no_grad():
            ...
            if len(z.shape) == 4:
                ...
                z_scaling_factor = z_scaling_factor.view(1, 1, -1)
                pre_act_scaling_factor = pre_act_scaling_factor.view(1, 1, -1)
                pre_weight_scaling_factor = pre_weight_scaling_factor.view(1, 1, -1)
            elif len(z.shape) == 2:
            ...
            if case == 1:
                if len(z.shape) == 4:
                    ...
                    identity_scaling_factor = identity_scaling_factor.view(1, 1, -1)
                    identity_weight_scaling_factor = identity_weight_scaling_factor.view(1, 1, -1)
                elif len(z.shape) == 2:
                ...
                # print("id scale/id.weight scale:",identity_scaling_factor.shape,identity_weight_scaling_factor.shape) #size[1],[1]
                # print("id scale:",identity_scaling_factor,identity_weight_scaling_factor)

            if case == 0:
                # print("zshape",z.shape)
                # print(pre_act_scaling_factor.shape,pre_weight_scaling_factor.shape)
                # print(";;;;;;;;;==========;;;;;;;;;;")
                # convert z from floating point to integer first
                if z.shape[1]==pre_weight_scaling_factor.shape[2]:
                    wz = z.transpose(1, 2)
                else:
                    wz =z

                z_int = torch.round(wz / pre_act_scaling_factor / pre_weight_scaling_factor)
                ...
                if len(z.shape) == 4:
                    ...
                    new_scale = new_scale.view(1, 1, -1)
                elif len(z.shape) == 2:
                    new_scale = transfer_fc_size(new_scale)

                m, e = batch_frexp(new_scale)
                # print("new scale:",new_scale.shape) #size [1,1,196/384/1536]
                # print("m/e:",m.shape,e.shape)
            ...
            # in case 1, the tensor z should be separeted into 2 parts, one from wy, another from wx
                ...
                # print(z.shape)
                # print(pre_act_scaling_factor.shape,pre_weight_scaling_factor.shape)
                # print(identity_scaling_factor.shape,identity_weight_scaling_factor.shape)
                # print("++++++++++++===========++++++++++")
                ...
                # print("new scale:",new_scale.shape) #size [1,1,1]
                # print("m1/e1:",m1.shape,e1.shape)
                ...
                # print("new scale:",new_scale.shape) #size [1,1,1]
                # print("m2/e2:",m2.shape,e2.shape)

                output2 = wy_int.type(torch.double) * m2.type(torch.double)

                output2 /= (2.0 ** e2)
                if z.shape[1]==pre_weight_scaling_factor.shape[2]:
                    output2 = torch.round(output2).transpose(1, 2)
                else:
